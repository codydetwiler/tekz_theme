...
/**
 * [tekz_product_title description]
 * @return [type] [description]
 */
function tekz_product_title() {
    return '<h6 class="tx-product-title shop-block_one-heading"><a href="' . get_the_permalink() . '">' . get_the_title() . '</a></h6>';
}

/**
 * [tekz_product_cart_button description]
 * @return [type] [description]
 */
function woocommerce_custom_sale_text($text, $post, $_product) {
    $show_discount_percentage = cs_get_option( 'show_discount_percentage', false );
    if($show_discount_percentage == false) {
        return '<span class="tx-product-badge tx-product-badge__sale off-tag position-absolute">'.__('Sell', 'tekz').'</span>';
    }
}


function tekz_woo_category() {
    global $product;
...
function tekz_get_price() {
    ob_start(); ?>
    <span class="woocommerce-Price-amount amount price"><?php print tekz_get_price_html(); ?></span>
    <?php
    return ob_get_clean();
}

function tekz_product_price( $product_id, $oldp = false ) {
...
function tekz_get_price_html() {
    global $product;
    return $product->get_price_html();
}

/**
 * [tekz_comment_rating description]
 * @param  [type] $rating [description]
 * @return [type]         [description]
 */
function tekz_comment_rating($rating)
{
...
add_filter('woocommerce_product_additional_information_heading', 'tekz_tab_heading');

add_filter('woocommerce_product_description_heading', 'tekz_tab_heading');

/**
 * [tekz_tab_heading description]
 * @param  [type] $heading [description]
 * @return [type]          [description]
 */
function tekz_tab_heading($heading)
{
    return '';
}

function tekz_woocommerce_get_sidebar()
{
...
function tekz_add_cart_button() {
    ...
    $custom_add_to_cart_text = cs_get_option('custom_add_to_cart_text', __('Purchase Now', 'tekz'));
...
function tekz_product_actions() {

    if(function_exists('tekz_quick_view_button') && TEKZ_CORE && TEKZ_WOOCOMMERCE_ACTIVED && class_exists( 'WPCleverWoosq' )) {
    ?>
        <div class="eye">
            <?php print tekz_quick_view_button(); ?>
        </div>
        <?php
    }
}

function tekz_woocommerce_template_single_price(){
    print tekz_get_price_html();
}
...
function tekz_related_products_limit() {
    global $product;

      $args['posts_per_page'] = 3;
      return $args;
}
add_filter( 'woocommerce_output_related_products_args', 'tekz_related_products_args', 20 );
function tekz_related_products_args( $args ) {
